...
| **Login-required / challenge gate** | The platform blocked unauthenticated access with a login modal, redirect (e.g. `authwall`), or CAPTCHA. | Status displays `Partial results · The platform requires attention: login or access check`. | Open the platform in the specific Chrome session or profile selected by `socai`, complete authentication or challenges, and verify browsing before re-running. |
| **Valid empty result** | The platform loaded successfully and the search executed cleanly, but returned 0 items. | `0 captured records` with status `partial` (`Partial results · Jev stopped without usable evidence.`). | Jev Social only marks a run `completed` when items exist. The search finished without matches; broaden or rephrase your query. |
...
Because resolution is not pinned to a single binary and may differ from what is on your current shell `PATH`, always test the exact binary resolved by Jev Social (found in the `socai.bin` field of `/api/status`):

```bash
# Inspect version of the resolved binary
"$RESOLVED_SOCAI_BIN" --version
```
...
- **Resolution**: Complete any active puzzle verification within the Chrome session/profile selected by `socai`. Test direct platform access using the resolved `socai` executable:
  ```bash
  "$RESOLVED_SOCAI_BIN" tiktok search "wearable AI" --num 4 --pretty
  ```
...
- **Supported Diagnostics**:
  ...
  - Test subcommand availability directly on the resolved executable:
    ```bash
    "$RESOLVED_SOCAI_BIN" linkedin --help
    ```
- **Resolution**: Sign into LinkedIn within the Chrome session/profile selected by `socai`. Once your authenticated session is active and `socai.capabilities.linkedin` is `true`, run research through Jev Social:
  ...
  ```bash
  "$RESOLVED_SOCAI_BIN" linkedin search "AI product managers" --num 4 --pretty
  ```
...
4. **Action History**: The "Steps chosen by Jev" list displays each operation executed along with its observed result and status.
...
If a run is manually cancelled (e.g. by navigating back to search, clicking back, or closing the stream):
- The abort signal immediately halts the active search and child processes before `evidenceReport()` and `saveRun()` execute.
- The stream terminates without emitting a final run result event, and the UI does not execute `renderRun()`.
- The run stops cleanly without producing a finalized or saved report, and report download is not available for cancelled runs.
